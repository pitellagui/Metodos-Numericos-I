...
b) Ache o valor de x que maximiza a função f(x) utilizando a busca da razão
áurea com aproximações iniciais de xl = -2 e xu = 4.

c) Ache o valor de x que maximiza a função f(x) utilizando interpolação
quadrática com aproximações iniciais de x1 = 1,75, x2 = 2 e x3 = 2,5.
"""
...
def func(x):
    return -(4*x - 1.8*x**2 + 1.2*x**3 - 0.3*x**4) # - para tornar um máximo um mínimo
