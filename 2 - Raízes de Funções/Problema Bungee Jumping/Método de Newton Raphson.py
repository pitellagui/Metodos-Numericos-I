...
Use o método de Newton Raphson para resolver o problema do Bungee Jumping.
"""
...
def dfunc(m, v, g, t, c_d):
  return 1/2*np.sqrt(g/(m*c_d))*np.tanh(np.sqrt((g*c_d)/m)*t)-g/(2*m)*t/(np.cosh(np.sqrt(g*c_d/m)*t)**2)


[m, fx, Epest, iter, nn, estimativa, E_pest] = Newton_Raphson(func, dfunc, x0, Eppara, maxit, v, g, t, c_d)
